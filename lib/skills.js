...
function cleanupCloneDest(dest) {
  if (fs.existsSync(dest)) {
    fs.rmSync(dest, { recursive: true, force: true });
  }
}

function tryGitClone(cloneUrl, branch, dest) {
  const branchArg = `-b ${branch}`;
  return runAllowFail(`git clone --depth 1 ${branchArg} "${cloneUrl}" "${dest}"`, { stdio: 'inherit' });
}

function setCanonicalOrigin(dest, httpsUrl) {
  if (httpsUrl) {
    runAllowFail(`git -C "${dest}" remote set-url origin "${httpsUrl}"`, { stdio: 'pipe' });
  }
}

function buildCloneAttempts(httpsUrl, mirrorUrl, mirrorFirst) {
  const mirrorAttempt = mirrorUrl
    ? { url: mirrorUrl, via: 'mirror', label: 'GitHub 镜像' }
    : null;
  const httpsAttempt = { url: httpsUrl, via: 'https', label: 'GitHub 直连' };

  if (mirrorFirst && mirrorAttempt) {
    return [mirrorAttempt, httpsAttempt];
  }
  if (mirrorAttempt) {
    return [httpsAttempt, mirrorAttempt];
  }
  return [httpsAttempt];
}

function gitClone(httpsUrl, sshUrl, branch, dest, mirrorPrefix, mirrorFirst) {
  const mirrorUrl = mirrorPrefix ? toGithubMirrorUrl(httpsUrl, mirrorPrefix) : null;
  const attempts = buildCloneAttempts(httpsUrl, mirrorUrl, mirrorFirst);

  for (let i = 0; i < attempts.length; i += 1) {
    if (i > 0) {
      log(`  ${attempts[i - 1].label} clone 失败，尝试 ${attempts[i].label}...`, 'yellow');
      cleanupCloneDest(dest);
    }
    log(`  clone ${attempts[i].url} → ${dest}`, 'blue');
    if (tryGitClone(attempts[i].url, branch, dest)) {
      if (attempts[i].via === 'mirror') {
        setCanonicalOrigin(dest, httpsUrl);
      }
      return { ok: true, via: attempts[i].via };
    }
  }

  if (!sshUrl) return { ok: false };

  log('  HTTPS/镜像 clone 失败，尝试 SSH...', 'yellow');
  cleanupCloneDest(dest);
  log(`  clone ${sshUrl} → ${dest}`, 'blue');
  if (tryGitClone(sshUrl, branch, dest)) {
...
function pullWithOrigin(dest, originUrl) {
  runAllowFail(`git -C "${dest}" remote set-url origin "${originUrl}"`, { stdio: 'pipe' });
  runAllowFail(`git -C "${dest}" fetch`, { stdio: 'inherit' });
  return runAllowFail(`git -C "${dest}" pull --ff-only`, { stdio: 'inherit' });
}

function gitSyncWithFallback(dest, sshUrl, httpsUrl, mirrorPrefix, mirrorFirst) {
  const canonicalHttps = httpsUrl || (isHttpsRemote(getOriginRemote(dest)) ? normalizeToHttps(getOriginRemote(dest)) : null);
  const mirrorUrl = mirrorPrefix && canonicalHttps ? toGithubMirrorUrl(canonicalHttps, mirrorPrefix) : null;

  if (canonicalHttps) {
    setCanonicalOrigin(dest, canonicalHttps);
  }

  const syncAttempts = buildCloneAttempts(
    canonicalHttps || httpsUrl,
    mirrorUrl,
    mirrorFirst,
  ).filter(attempt => attempt.url);

  for (let i = 0; i < syncAttempts.length; i += 1) {
    if (i > 0) {
      log(`  ${syncAttempts[i - 1].label} pull 失败，尝试 ${syncAttempts[i].label}...`, 'yellow');
    }
    if (pullWithOrigin(dest, syncAttempts[i].url)) {
      if (syncAttempts[i].via === 'mirror' && canonicalHttps) {
        setCanonicalOrigin(dest, canonicalHttps);
      }
      return { ok: true, via: syncAttempts[i].via };
    }
    if (canonicalHttps) {
      setCanonicalOrigin(dest, canonicalHttps);
    }
  ...
  if (!remote || !sshUrl) {
    if (canonicalHttps) setCanonicalOrigin(dest, canonicalHttps);
    return { ok: false };
  }

  log('  HTTPS/镜像 pull 失败，尝试切换 SSH remote...', 'yellow');
...
function getGithubMirrorOptions() {
  const config = loadConfig();
  return {
    mirrorPrefix: resolveGithubMirrorPrefix(config),
    mirrorFirst: preferGithubMirrorFirst(config),
  };
}

function gitCloneWithFallback(skill, dest) {
  const { httpsUrl, sshUrl, branch } = resolveUrls(skill);
  const { mirrorPrefix, mirrorFirst } = getGithubMirrorOptions();
  const result = gitClone(httpsUrl, sshUrl, branch, dest, mirrorPrefix, mirrorFirst);
  if (!result.ok) {
    log(`✗ skill "${skill.name}" clone 失败（HTTPS、镜像与 SSH 均不可用）`, 'red');
    process.exit(1);
  }
  return result.via;
}
...
function installOne(skill) {
  ...
  const { httpsUrl, sshUrl } = resolveUrls(skill);
  const { mirrorPrefix, mirrorFirst } = getGithubMirrorOptions();
  ...
  if (hasRepo) {
    ...
    const syncResult = gitSyncWithFallback(dest, sshUrl, httpsUrl, mirrorPrefix, mirrorFirst);
    syncSkillLink(skill.name);
    if (syncResult.ok) {
  ...
  const action =
    via === 'ssh' ? 'cloned-ssh' : via === 'mirror' ? 'cloned-mirror' : 'cloned';
  return { name: skill.name, action, via };
}
...
function upgradeOne(skill) {
  ...
  if (!hasRepo) {
    ...
    const via = gitCloneWithFallback(skill, dest);
    syncSkillLink(skill.name);
    return { name: skill.name, action: via === 'ssh' ? 'cloned-ssh' : via === 'mirror' ? 'cloned-mirror' : 'cloned', via };
  }
