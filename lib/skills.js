...
const { toGithubMirrorUrl, resolveGithubMirrorPrefix } = require('./github-mirror');
const {
  getAgentsSkillsDir,
...
function gitClone(httpsUrl, sshUrl, branch, dest, mirrorPrefix) {
  const mirrorUrl = mirrorPrefix ? toGithubMirrorUrl(httpsUrl, mirrorPrefix) : null;

  log(`  clone ${httpsUrl} → ${dest}`, 'blue');
  if (tryGitClone(httpsUrl, branch, dest)) {
    return { ok: true, via: 'https' };
  }

  if (mirrorUrl) {
    log('  HTTPS clone 失败，尝试 GitHub 镜像...', 'yellow');
    cleanupCloneDest(dest);
    log(`  clone ${mirrorUrl} → ${dest}`, 'blue');
    if (tryGitClone(mirrorUrl, branch, dest)) {
      setCanonicalOrigin(dest, httpsUrl);
      return { ok: true, via: 'mirror' };
    }
  }
...
function gitSyncWithFallback(dest, sshUrl, httpsUrl, mirrorPrefix) {
  ...
  if (mirrorUrl && canonicalHttps) {
    log('  HTTPS pull 失败，尝试 GitHub 镜像 remote...', 'yellow');
    runAllowFail(`git -C "${dest}" remote set-url origin "${mirrorUrl}"`, { stdio: 'pipe' });
    runAllowFail(`git -C "${dest}" fetch`, { stdio: 'inherit' });
    if (runAllowFail(`git -C "${dest}" pull --ff-only`, { stdio: 'inherit' })) {
...
function installOne(skill) {
  ...
  if (hasRepo) {
    ...
    const syncResult = gitSyncWithFallback(dest, sshUrl, httpsUrl, mirrorPrefix);
    syncSkillLink(skill.name);
    if (syncResult.ok) {
