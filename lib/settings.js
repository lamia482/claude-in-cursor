...
const { buildProfileProxyMeta } = require('./proxy-config');
...
function writeSettings(apiKey, config) {
  ...
  settings.model = config.claudeModelTier || 'opus';
  settings.effortLevel = config.effortLevel || 'max';
...
function writeProfile(apiKey, config) {
  ...
  const profile = {
    ...
    claudeInCursor: buildProfileProxyMeta(config),
  };
...
function syncRuntimeSettings(config) {
  const settingsPath = getSettingsPath();
  if (!fs.existsSync(settingsPath)) {
    return false;
  }
