...
const { ensureProxyRunning } = require('./proxy-lifecycle');
const { syncRuntimeSettings } = require('./settings');

const PACKAGE = '@supertiny99/cc-switch';

function getVersion() {
  const cmd = resolveCcsCommand();
  if (cmd) {
    return runQuiet(`"${cmd}" --version`);
  }
...
async function use(profileId, { interactive = false } = {}) {
  const cmd = getCommand();
  if (!cmd) {
  ...
  await ensureProxyRunning(loadConfig());
  syncRuntimeSettings(loadConfig());
  return true;
}
