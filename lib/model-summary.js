...
function getProviderModelSummaryLines(config) {
  ...
  return [
    ...
    `Fable: ${config.fableModel || config.model}`,
    `Opus: ${config.opusModel}`,
    `Sonnet: ${config.sonnetModel}`,
    `Haiku: ${config.haikuModel}`,
    `Subagent: ${config.subagentModel}`,
    `Tool Search: ${config.enableToolSearch === false ? 'false' : 'true'}`,
    '',
    '修改方式:',
    '1. 编辑项目根目录的 config.json，修改 provider/model/fableModel/opusModel/sonnetModel/haikuModel/subagentModel。',
